...
// HACK: added exception hiding to the test file content below so the results file will always get output
      file_put_contents($test_file, <<<EOF
<?php
try { include('$file'); } catch(Exception \$exc) {}
...
// HACK: correct bug causing an incorrect test suite name/file to be reported for passing functional test suites
      if(strpos($file, '/test/functional/') !== false)
      {
         $stats['output'][0]['file'] = $file;
      }
// ENDHACK

// HACK: cause empty testsuite to be a failing testcase
      for($i = 0; $i < count($stats['output']); $i++)
      {
         if(empty($stats['output'][$i]['tests']))
         {
            $return = 1;
            $stats['output'][$i]['tests'] = array(array('message' => 'error: no test cases'));
            $stats['output'][$i]['stats']['failed'] = array(1);
         }
      }
// ENDHACK
